...
class ReplyReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        Debug.d("TAG_message", "onReceive:notificationId $notificationId")
        ...
        if (intent.action == "MARK_AS_READ") {
            ...
            Debug.d("TAG_message", "onReceive:name ${senderUser.name}")
            Debug.d("TAG_message", "onReceive:uid ${senderUser.uid}")
            chatRepository.getMessages(senderUser.uid) {
                if (!markAsRead) {
                    markAsRead = true
                    Debug.d("TAG_message", "onReceive:uid ${it.size}")
            ...
            Debug.d("TAG_message", "onReceive:replyText $replyText")

            if (!replyText.isNullOrEmpty()) {
                // Send Reply to FireStore or Firebase Database
                Debug.d("TAG_message", "onReceive:replyText11 $replyText")

                // Manually get ViewModel using Hilt EntryPoint
                val myUserProfile =
                    PreferenceClass(context).getPrefValue("my_profile_data", "").toString()

                Debug.d("TAG_message", "onReceive:myUserProfile $myUserProfile")
